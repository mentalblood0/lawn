...
impl VariableDataPool {
    ...
    ) -> VariableDataPoolUpdatingIterator<'a, D, P>
    where
        D: Iterator<Item = Vec<u8>>,
        P: Iterator<Item = u64>,
    {
        VariableDataPoolUpdatingIterator {
            variable_data_pool: self,
            data_to_add,
            pointers_to_data_to_remove,
            container_size_index_to_encoded_data_to_add: [const { Vec::new() };
                CONTAINERS_SIZES_COUNT],
            container_size_index_to_data_to_add_initial_indices: [const { Vec::new() };
                CONTAINERS_SIZES_COUNT],
        }
    }
}

pub struct VariableDataPoolUpdatingIterator<
    'a,
    D: Iterator<Item = Vec<u8>>,
    P: Iterator<Item = u64>,
> {
    variable_data_pool: &'a mut VariableDataPool,
    data_to_add: D,
    pointers_to_data_to_remove: P,
    container_size_index_to_encoded_data_to_add: [Vec<Vec<u8>>; CONTAINERS_SIZES_COUNT],
    container_size_index_to_data_to_add_initial_indices: [Vec<usize>; CONTAINERS_SIZES_COUNT],
}

// impl<'a, D, P> FallibleIterator for VariableDataPoolUpdatingIterator<'a, D, P>
// where
//     D: Iterator<Item = Vec<u8>>,
//     P: Iterator<Item = u64>,
// {
