...
impl Index {
    pub fn write_header(mut file: &fs::File, header: &IndexHeader) -> Result<u64> {
        ...
        let result =
            bincode::encode_into_std_write(header, &mut writer, bincode::config::standard())
                .with_context(|| "Can not encode header")? as u64;
        writer
            .flush()
            .with_context(|| "Can not flush new index file header")?;
        Ok(result)
    }
    ...
    pub fn get(&self, record_index: u64) -> Result<Option<u64>> {
        let mut buffer = vec![0; self.header.record_size as usize];
        if self
            .file
            .read_exact_at(
                &mut buffer,
            ...
            .is_ok()
        {
            ...
            for byte in buffer.into_iter().rev() {
    ...
    pub fn iter(&self, from_record_index: u64, backwards: bool) -> Result<IndexIterator> {
        ...
        Ok(IndexIterator {
            ...
            buffer: vec![0; self.header.record_size as usize],
        ...
        if self.backwards {
            if !self.first_read {
                self.reader
                    ...
                    .with_context(|| "Can not seek backwards while iterating index")?;
            }
            self.reader
                .read_exact(&mut self.buffer)
                .with_context(|| "Can not read index record (data id) while iterating index")?;
